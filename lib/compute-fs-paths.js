...
function computeFsPaths(opts) {
  ...
  if (opts.drivers.chrome) {
    fsPaths.chrome = {
      installPath: path.join(opts.basePath, 'chromedriver', opts.drivers.chrome.version + '-' + opts.drivers.chrome.arch + '-' + process.platform + '-chromedriver')
    };
  }
  if (opts.drivers.ie) {
    fsPaths.ie = {
  ...
  if (opts.drivers.firefox) {
    fsPaths.firefox = {
      installPath: path.join(opts.basePath, 'geckodriver', opts.drivers.firefox.version + '-' + opts.drivers.firefox.arch + '-' + process.platform +  '-geckodriver')
    };
  }
